...
public class KFiles {
	...
	private static ArrayList<String> FiletoString(ArrayList<File> NamesList){
		...
		NamesListString.add(NamesList.get(i).toString().replace(NamesList.get(i).toString().substring(0,lastindex+1), ""));
		Addresses.add(NamesList.get(i).toString());
		RootAddresses.add(Addresses.get(i).replace(Addresses.get(i).toString().substring(lastindex+1 , Addresses.get(i).toString().length()), ""));
		litr.next();
		i++;
